...
#Model13
Model13<- nnetar(ts_load_train,
                  p=1,
                  P=0,
                  xreg=fourier(ts_load_train, K=c(2,18)))
...
#NN K=2,24
Model13_for <- forecast(Model13, h=n_for,xreg=fourier(ts_load_train,K=c(2,18),h=n_for))
autoplot(Model13_for)
Model13_score <- accuracy(Model10_for$mean, ts_load_test)
...
all_scores <- as.data.frame(rbind(SES_score, SSES_score, ETS_score, Model7_score, Model8_score, Model9_score, Model10_score, Model12_score, Model13_score, Model14_score))
row.names(all_scores) <- c("SES", "SSES", "STL+ETS", "A+F,K=c(2,12)", "A+F,K=c(2,24)", "SNaive", "NN K=c(2,12)", "STL", "NN K=c(2,18)", "A+F+Temp,K=(2,24)")
