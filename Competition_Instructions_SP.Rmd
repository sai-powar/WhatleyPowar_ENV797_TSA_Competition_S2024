...
```{r}
#old code do not run 
#creating and plotting non-seasonal series
deseasonal_load <- seasadj(decompose_load)
plot(deseasonal_load)
...
plot_grid(
  autoplot(Acf(ts_load, lag = 40, plot=FALSE), 
                main = "Orginal Load"),
  autoplot(Acf(deseasonal_load, lag = 40, plot=FALSE),  
                  main = "Deseason Load")
)

print("Results for ADF test/n")
print(adf.test(deseasonal_load))
...
all_scores <- as.data.frame(rbind(SARIMA_score, SES_score, SSES_score, ETS_score, Model7_score, Model8_score))
row.names(all_scores) <- c("SARIMA", "SES", "SSES", "STL+ETS", "A+F,K=c(2,12)", "A+F,K=c(2,24)")
